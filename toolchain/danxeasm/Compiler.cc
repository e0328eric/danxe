...
using namespace danxe::danxeasm;
...
static Err parseInteger(Value* output, const char* start, const char* end, int base);
static Err parseUInteger(Value* output, const char* start, const char* end, int base);
...
#define WRITE_FILE(_ptr, _nitems)                                                   \
    ...
    // First run the main function of the danxeasm assembly compiler
    Err err = Err::Ok;
    ...
    Metadata metaToWrite{MAGIC_NUMBER, DANXE_VM_VERSION, (uint64_t)(32 + mGlobalMemCount + padding),
...
Err Compiler::parseGlasm() {
    ...
    for (size_t jumpIdx = 0; jumpIdx < mJumpsCount; ++jumpIdx) {
        ...
        mInst[mJumps[jumpIdx].inst].operand = Value{(uint64_t)mLabels[labelIdx].progLine};
    }
...
Err Compiler::parseOpcode() {
    Err err = Err::Ok;
    auto opcode = strToOpcode(mStart, (size_t)(mPos0 - mStart));
    ...
    // add new instruction (and later, parse loop)
    ...
    if (opType == OperandType::LoopOperand) {
        ...
        mJumps[mJumpsCount].labelName = std::string_view{mStart, (size_t)(mPos0 - mStart)};
        ++mJumpsCount;
    }
...
Err Compiler::parseOperand(Value* output, const Opcode& opcode) {
    ...
    std::string_view operandStr{mStart, (size_t)(mPos0 - mStart)};
    ...
    if (operandStr == "null")
        ...
        *output = Value{false};
        ...
        err = parseUInteger(output, mStart, mPos0, operandTy.radix);
    else if (operandTy.isFloat)
        err = parseFloat(output, mStart, mPos0);
    else
        err = parseInteger(output, mStart, mPos0, operandTy.radix);
...
Err Compiler::parseLoopLabel() {
    mStart = ++mPos0;
    while (!isEitherChar(*mPos0, '>', '\0')) {
        nextChar();
    }

    const char* i = mStart;
    while (i != mPos0 && isLabelNameLetter(*i)) {
        ++i;
    }
    ...
    std::string_view loopLabel{mStart, (size_t)(mPos0 - mStart)};
...
Err Compiler::parseChar(Value* output, const Opcode& opcode) {
    ...
    if ((size_t)(mPos0 - mStart) >= 5)
        return Err::ParseCharErr;

    memcpy((void*)buffer, mStart, (size_t)(mPos0 - mStart));
    buffer[mPos0 - mStart] = '\0';
...
// TODO(#8): danxeasm cannot parse escape character in the proper way
Err Compiler::parseString(Value* output, const Opcode& opcode) {
    if (*mPos0 != '"' || !hasStringOperand(opcode))
    ...
    if ((size_t)(mPos0 - mStart) >= GLOBAL_MEM_CAPACITY)
        return Err::GlobalMemoryOverflowErr;

    size_t memStart = mGlobalMemCount;
    memcpy((void*)(mGlobalMem + mGlobalMemCount), mStart + 1, (size_t)(mPos0 - mStart - 1));
    mGlobalMemCount += (size_t)(mPos0 - mStart - 1);
...
static Err parseInteger(Value* output, const char* start, const char* end, int base) {
    ...
    int64_t value = 0;
    if (auto [result, ec] = std::from_chars(start, end, value, base); ec == std::errc()) {
        *output = Value{value};
        return Err::Ok;
    }
...
static bool parseUIntOpcode(const Opcode& opcode) {
    return opcode == Opcode::Dup || opcode == Opcode::Swap || opcode == Opcode::Rotate ||
