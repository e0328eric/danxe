...
#define PADDING_STRING_I "__DANXEASM_IWUHNDXAXS__"
...
// A static member
std::vector<std::filesystem::path> Preprocessor::mParents;
...
Preprocessor::Preprocessor(const std::filesystem::path& mainFilePath, const char* source)
    : mMainPath(),
      mSource(source),
      mStart(source),
      mCurrent(source),
        ...
        switch (*mCurrent) {
            case '%':
                ...
                if (strncmp(mCurrent, "include", 7) == 0) {
                    if ((includeHandle &
                         (FIRST_INCLUDE_MACRO_FOUND | APPEND_MAIN_LOCATION_FINISHED)) == 0) {
                        result.append("jmp ");
            ...
            default:
                if (mDefinedFstLetter[static_cast<size_t>(*mCurrent)]) {
                    ...
                    if (includeHandle == (FIRST_INCLUDE_MACRO_FOUND | NON_MACRO_CHAR_FOUND)) {
                        result.append("\n<");
                        result.append(makeHashString(mMainPath.c_str()));
                        result.append(">\n");
...
Err Preprocessor::parseIncludes(std::string& string) {
    ...
    if (std::find(mParents.begin(), mParents.end(), filename) != mParents.end()) {
        return Err::IncludeCycleFoundErr;
    } else {
        mParents.emplace_back(filename);
    }

    if (mParents.size() >= INCLUDE_THRESHOLD) {
        return Err::IncludeCycleFoundErr;
    }
...
Err Preprocessor::pluginDefine(std::string& string, char& includeHandle) {
    ...
    // the identifier
    if (!mIsSorted) {
        std::sort(mIdentPairs.begin(), mIdentPairs.end(),
                  [](auto x, auto y) { return x.identifier.size() > y.identifier.size(); });
        mIsSorted = true;
    }

    for (i = 0; i < identPairsLen; ++i) {
        pair = &mIdentPairs[i];
        if (strncmp(mCurrent, pair->identifier.data(), pair->identifier.size()) == 0) {
            break;
        }
    }
