...
constexpr const char* USAGE_MSG = "USAGE: danxevm [--debug] <input-binary>";
...
int main(int argc, char* argv[]) {
    ...
    switch (argc) {
        ...
        case 2: {
            ...
            if ((err = danxe::danxevm::readFile(&sourceStr, argv[1])) != Err::Ok) {
                std::cerr << err << std::endl;
                return 1;
            }

            danxe::danxevm::Vm vm{sourceStr};
            err = vm.run();
        ...
        case 3: {
            ...
            if ((err = danxe::danxevm::readFile(&sourceStr, argv[2])) != Err::Ok) {
                std::cerr << err << std::endl;
                return 1;
            }

            danxe::danxevm::Debugger debugger{sourceStr};
            err = debugger.run();
