...
Err Debugger::run() {
    ...
    while (!Vm::isHalt() && err == Err::Ok && !isQuit) {
        ...
        if (!line) {
            linenoiseFree(line);
            return DanxeVmErr::DebuggerLinenoiseErr;
        }
...
Err Debugger::runArgument(bool* isQuit, char ch) {
    ...
    switch (ch) {
        ...
        case 'c':
            ...
            std::cout << "<Data Stack>\n";
            std::cout << "[ ";
            for (size_t i = 0; i < Vm::getDataSp(); ++i)
                std::cout << Vm::getDataStack()[i] << " ";
            std::cout << "]\n";
            std::cout << "<Call Stack>\n";
            std::cout << "[ ";
            for (size_t i = 0; i < Vm::getCallSp(); ++i)
                std::cout << Vm::getCallStack()[i] << " ";
            std::cout << "]\n" << std::endl;
            break;
