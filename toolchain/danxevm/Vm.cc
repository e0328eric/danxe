...
using Err = danxe::danxevm::DanxeVmErr;
...
// TODO: split into each stack error message
Err Vm::runInst() {
...
#define CHECK_DATA_STACK_OVERFLOW(_n)            \
    if (mDataSp + (_n) >= DATA_STACK_CAPACITY) { \
        err = Err::StackOverflowErr;             \
        break;                                   \
    }

#define CHECK_DATA_STACK_UNDERFLOW(_n) \
    if (mDataSp < (_n)) {              \
        err = Err::StackUnderflowErr;  \
        break;                         \
    }

#define CHECK_CALL_STACK_OVERFLOW(_n)            \
    if (mCallSp + (_n) >= CALL_STACK_CAPACITY) { \
        err = Err::StackOverflowErr;             \
        break;                                   \
    }

#define CHECK_CALL_STACK_UNDERFLOW(_n) \
    if (mCallSp < (_n)) {              \
        err = Err::StackUnderflowErr;  \
        break;                         \
    }
...
#define CHECK_IS_VALID_RELATIVE_JUMP(_n)                                            \
    if ((uint64_t)((int64_t)mIp + (_n)) >= mInstLen || ((int64_t)mIp - (_n)) < 0) { \
        err = Err::InvalidJumpErr;                                                  \
        break;                                                                      \
    }
...
#define CAST_VALUE(_from, _fromFnt, _to)   \
    ...
    switch (opcode) {
        ...
        case Opcode::Rotate: {
            ...
            auto tmp = mStack[mSp - 1 - value];
            memmove(mStack + (mSp - 1 - value), mStack + (mSp - value), sizeof(Value) * value);
            mStack[mSp - 1] = tmp;
            ++mIp;
            break;
        }
        ...
        case Opcode::RelativeJmpGreatEq: {
            CONVERT_OPERAND(goInto, int64_t, getIntVal);
            CHECK_IS_VALID_RELATIVE_JUMP(goInto);
            CHECK_STACK_UNDERFLOW(2);
        ...
        case Opcode::Call: {
            CONVERT_OPERAND(goInto, uint64_t, getUIntVal);
            CHECK_IS_VALID_JUMP(goInto);
            CHECK_CALL_STACK_OVERFLOW(1);

            mCallStack[mCallSp++] = mIp + 1;
            mIp = goInto;
        ...
        case Opcode::Return: {
            CHECK_CALL_STACK_UNDERFLOW(1);

            size_t addr = mCallStack[--mCallSp];
            mIp = addr;
        ...
        case Opcode::Store:
            CHECK_STACK_UNDERFLOW(1);
            CHECK_DATA_STACK_OVERFLOW(1);
            mDataStack[mDataSp++] = mStack[--mSp];
            ++mIp;
            break;

        case Opcode::Load:
            CHECK_STACK_OVERFLOW(1);
            CHECK_DATA_STACK_UNDERFLOW(1);
            mStack[mSp++] = mDataStack[--mDataSp];
            ++mIp;
            break;
...
const Value* Vm::getDataStack() const {
    return mDataStack;
}

size_t Vm::getDataSp() const {
    return mDataSp;
}

const size_t* Vm::getCallStack() const {
    return mCallStack;
}

size_t Vm::getCallSp() const {
    return mCallSp;
}
